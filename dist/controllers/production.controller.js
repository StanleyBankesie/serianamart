...
export const listWorkOrders = async (req, res, next) => {
  try {
    ...
    const items = await query(
      `SELECT id, work_order_no, work_order_date, status, created_at FROM prod_work_orders 
...
export const getWorkOrderById = async (req, res, next) => {
  try {
    ...
    const items = await query(
      `SELECT * FROM prod_work_orders WHERE id = :id AND company_id = :companyId AND branch_id = :branchId LIMIT 1`,
...
export const createWorkOrder = async (req, res, next) => {
  try {
    ...
    const result = await query(
      `INSERT INTO prod_work_orders (company_id, branch_id, work_order_no, work_order_date, status, remarks)
