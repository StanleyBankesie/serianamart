...
router.put(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      if (String(body.work_status).toUpperCase() === "COMPLETED") {
        finalStatus = "POSTED";
      }
...
const anchorRegex = /router\.get\(\s*"\/service-executions\/:id"/;
code = code.replace(anchorRegex, putEndpointCode + '\n$&');
...
// We should also replace the bug in nextServiceExecutionNo that was reverted when we ran git restore
code = code.replace(
