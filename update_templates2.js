...
const newCode = `  const commonHead = \`
<style>
  :root { --text: #000; }
...
function replaceContent(path) {
  ...
  content = content.replace(/const commonHead = `[\s\S]*?  if \(type === "delivery-note"\) \{/, newCode + '  if (type === "delivery-note") {');
  fs.writeFileSync(path, content);
}

replaceContent("c:/Users/stanl/OneDrive/Documents/serianamart/server/routes/documents.routes.js");
replaceContent("c:/Users/stanl/OneDrive/Documents/serianamart/client/src/pages/modules/administration/templates/DocumentTemplatesPage.jsx");

async function runDb() {
  const c = await mysql.createConnection({host:'localhost', user:'seriana', password:'Origen@tor123', database:'seriana_db'});
  
  let head = newCode.split('const commonHead = \`')[1].split('\`;')[0];
  
  let sHtmlRaw = newCode.split('if (type === "sales-order") {')[1].split('if (type === "invoice") {')[0];
  let sHtml = sHtmlRaw.split('return \`\\n\\${commonHead}\\n')[1].split('\`;\\n  }')[0];
  sHtml = head + sHtml;
  
  let iHtmlRaw = newCode.split('if (type === "invoice") {')[1];
  let iHtml = iHtmlRaw.split('return \`\\n\\${commonHead}\\n')[1].split('\`;\\n  }')[0];
  iHtml = head + iHtml;

  await c.query('UPDATE document_templates SET html_content = ? WHERE document_type = "sales-order" AND is_default = 1', [sHtml]);
  await c.query('UPDATE document_templates SET html_content = ? WHERE document_type = "invoice" AND is_default = 1', [iHtml]);
