 ...
 * Complete Enterprise Physical Stock Take Management, Counting, Variance Reconciliation, and Audit Workspace.
 */
...
const STATUS_STEPS = [
  ...
  { key: "COUNTING", label: "2. Physical Counting" },
  { key: "UNDER_REVIEW", label: "3. Review & Discrepancies" },
...
export default function StockTakeForm() {
  ...
  // Setup form data for new stock take
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setCategories(Array.isArray(catRes.data?.items) ? catRes.data.items : []);
      } catch {}
    })();
  }, []);
  ...
  const handleCreateSubmit = async (e) => {
    ...
    if (!createForm.warehouse_id) {
      toast.error("Please select a warehouse for the stock take.");
      return;
    }
    ...
    try {
      ...
      navigate(`/inventory/stock-take/${res.data?.id}`);
    } catch (err) {
  ...
  const handleStartCount = async () => {
    ...
    try {
      ...
      toast.success(`Count started. Baseline snapshot captured for ${res.data?.total_items} items.`);
  ...
  const handleSubmitCount = async () => {
    ...
    if (unreasonedVariances.length > 0) {
      toast.warning(`Please select a variance reason for all ${unreasonedVariances.length} discrepant items before submitting.`);
    ...
    try {
      ...
      toast.success(`Count submitted for review (${res.data?.total_variance_items} discrepancies recorded)`);
      await fetchStockTakeDetails();
    } catch (err) {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      toast.success("Stock Take approved. Authorized for inventory adjustment posting.");
      await fetchStockTakeDetails();
    } catch (err) {
    ...
    try {
      ...
      toast.success(
        res.data?.adjustment_no
          ? `Inventory Adjustment ${res.data.adjustment_no} posted! Stock balances updated in ledger.`
          : "Stock Take closed. All inventory items matched system baseline.",
      );
  ...
  // Print Physical Count Sheet (Blank Physical Column for Clipboard Count)
  async function printCountSheet() {
    ...
    let y = applyPdfHeader(doc, headerInfo, {
      ...
      kpis: [
        { label: "SCOPE ITEMS", value: String(details.length), color: [59, 130, 246] },
        { label: "STATUS", value: stockTake?.status || "COUNTING", color: [100, 116, 139] },
      ],
    });
    ...
    details.forEach((d, idx) => {
      ...
      // Underline / Box for manual physical count entry
      doc.rect(pageW - margin - 25, y - 3.5, 25, 4.5);
      y += 6;
    });

    applyPdfFooter(doc);
    doc.save(`stock-count-sheet-${stockTake?.stock_take_no || id}.pdf`);
  }
  ...
  async function printVarianceReport() {
    ...
    let y = applyPdfHeader(doc, headerInfo, {
      title: "STOCK TAKE VARIANCE & RECONCILIATION REPORT",
      subtitle: `Stock Take: ${stockTake?.stock_take_no || "-"} | Warehouse: ${stockTake?.warehouse_name || "All"} | Status: ${stockTake?.status}`,
      kpis: [
        { label: "TRACKED ITEMS", value: String(summary.totalLines), color: [59, 130, 246] },
        { label: "SHORTAGES", value: `${summary.shortagesCount} items (GH ${summary.totalShortageVal.toFixed(2)})`, color: [239, 68, 68] },
        { label: "SURPLUSES", value: `${summary.surplusesCount} items (GH ${summary.totalSurplusVal.toFixed(2)})`, color: [16, 185, 129] },
        { label: "NET VARIANCE", value: `${headerInfo.currPrefix}${summary.netVarianceVal.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, color: [245, 158, 11] },
      ],
    });
    ...
    doc.text("SYSTEM QTY", 115, y + 4.2, { align: "right" });
    ...
    applyPdfFooter(doc);
    doc.save(`stock-variance-report-${stockTake?.stock_take_no || id}.pdf`);
  }

  // ==========================================
  // RENDER: 1. NEW STOCK TAKE SETUP SCREEN
  // ==========================================
  if (isNew) {
    return (
            ...
            <div className="flex justify-between items-center">
              <div>
                <h1 className="text-2xl font-bold flex items-center gap-2">
                  <ClipboardCheck className="w-7 h-7" /> Create Physical Stock Take
                </h1>
                <p className="text-sm mt-1 opacity-90">
                  Initialize a controlled inventory verification audit and define counting scope
                </p>
              </div>
            ...
            <form onSubmit={handleCreateSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    The specific warehouse location where physical count will be conducted.
                  </p>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    required
                  />
                  <p className="text-xs text-slate-500 mt-1">Scheduled date of physical count execution.</p>
                </div>
                ...
                <div>
                  ...
                  >
                    <option value="FULL_COUNT">Full Warehouse Count (All items)</option>
                    <option value="CATEGORY_COUNT">Category / Group Count</option>
                    <option value="CYCLE_COUNT">Cycle Count (Sample verification)</option>
                  </select>
                </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="e.g. End of Month Physical Stock Take Q3, counted by team A & B..."
                  value={createForm.remarks}
                  onChange={(e) => setCreateForm({ ...createForm, remarks: e.target.value })}
                />
              </div>

              <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-lg border border-slate-200 dark:border-slate-800 text-xs text-slate-600 dark:text-slate-400 space-y-1">
                <div className="font-semibold text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
                  <HelpCircle size={14} className="text-brand" /> ERP Inventory Control Rule:
                </div>
                <p>
                  1. Stock Take numbering is auto-generated following ERP sequence (`STK-000001`).
                </p>
                <p>
                  2. Upon clicking "Create & Open Workspace", the audit document is saved in <strong>Draft</strong> status.
                </p>
                <p>
                  3. The immutable inventory baseline snapshot will be captured when counting is initiated.
                </p>
              </div>
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-slate-200 dark:border-slate-700">
                ...
                >
                  {actionLoading ? "Creating..." : "Create & Open Workspace"} <ArrowRight size={14} />
                </button>
              </div>
            </form>
          </div>
    ...
    <div className="space-y-6 max-w-7xl mx-auto p-4 sm:p-6">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
            <div>
              ...
              <p className="text-sm mt-1 opacity-90 flex flex-wrap items-center gap-x-4 gap-y-1">
                <span>Warehouse: <strong>{stockTake?.warehouse_name || "All Warehouses"}</strong></span>
                <span>Date: <strong>{stockTake?.count_date ? String(stockTake.count_date).slice(0, 10) : "-"}</strong></span>
                <span>Scope: <strong>{stockTake?.count_type || "FULL_COUNT"}</strong></span>
            ...
            <div className="flex flex-wrap gap-2 items-center">
              <button onClick={() => navigate("/inventory/stock-take")} className="btn btn-secondary text-xs">
                Back to List
              </button>
              <button
                onClick={printCountSheet}
                className="btn btn-outline btn-sm text-white border-white/30 hover:bg-white/10 flex items-center gap-1.5 text-xs"
                title="Print Blank Count Sheet for Clipboard physical counting"
              >
              ...
              <button
                ...
                className="btn btn-outline btn-sm text-white border-white/30 hover:bg-white/10 flex items-center gap-1.5 text-xs"
                title="Export Variance Reconciliation Report"
              >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-6 gap-3">
        <div className="card p-3 border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-800 rounded-lg text-center">
          <div className="text-xs text-slate-500 font-semibold uppercase">Total Items</div>
        ...
        <div className="card p-3 border border-amber-200 dark:border-amber-900/50 bg-amber-50/40 dark:bg-amber-950/20 rounded-lg text-center">
          <div className="text-xs text-amber-600 dark:text-amber-400 font-semibold uppercase">Net Variance Value</div>
      ...
      <div className="card p-4 border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-800 rounded-lg shadow-sm">
        <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-3">
          <div className="text-sm font-medium text-slate-700 dark:text-slate-300 flex items-center gap-2">
            <span className="font-bold text-brand">Current Workflow Action:</span>
            {currentStatus === "DRAFT" && "Initiate physical count to capture snapshot baseline."}
            {currentStatus === "COUNTING" && "Enter physical quantities into table and save draft or submit for review."}
            {currentStatus === "UNDER_REVIEW" && "Review variance discrepancies and reasons before approval."}
            {currentStatus === "RECOUNT_REQUIRED" && "Recount in progress. Re-verify physical quantities."}
            {currentStatus === "APPROVED" && "Stock Take approved. Ready to post atomic inventory adjustment."}
            {isPosted && `Completed. Inventory adjustment ${stockTake?.adjustment_no || ""} posted to ledger.`}
          </div>
          ...
          <div className="flex flex-wrap gap-2 items-center justify-end">
            {currentStatus === "DRAFT" && (
              ...
              >
                <Play size={14} /> Start Count & Capture Baseline
              </button>
            )}
      ...
      <div className="card shadow-sm border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-800 rounded-lg p-5">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4 mb-4 pb-4 border-b border-slate-200 dark:border-slate-700">
          <div className="flex flex-wrap gap-2">
            ...
            >
              Matched Items ({summary.matchedCount})
            </button>
            <button
        ...
        {/* TAB 1, 2, 3: ITEMS COUNTING TABLE */}
        {activeTab !== "LOGS" ? (
            ...
            <table className="table w-full text-left">
              <thead className="sticky top-0 bg-slate-100 dark:bg-slate-800 z-10 text-xs uppercase text-slate-600 dark:text-slate-300 border-b border-slate-200 dark:border-slate-700">
                <tr>
                  ...
                  <th className="py-2.5 px-3 text-right" title="Baseline quantity captured at count start">System Snapshot</th>
                  <th className="py-2.5 px-3 text-right" title="Net movements (sales/receipts) during counting">Movements</th>
                  <th className="py-2.5 px-3 text-right" title="System Qty + Interim Movements">Expected Qty</th>
                  ...
                  <th className="py-2.5 px-3" style={{ minWidth: "180px" }}>Discrepancy Reason</th>
                ...
                {!filteredDetails.length ? (
                  <tr>
                    <td colSpan="10" className="text-center py-10 text-slate-500">
                      {currentStatus === "DRAFT"
                        ? "Stock Take is in Draft status. Click 'Start Count & Capture Baseline' above to initialize counting lines."
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl max-w-md w-full p-6 space-y-4 border border-slate-200 dark:border-slate-700">
            ...
            <p className="text-xs text-slate-600 dark:text-slate-400">
              The stock take will transition to <strong>RECOUNT_REQUIRED</strong> status. Existing physical counts will be preserved into audit history, and the count team will be requested to re-verify the physical stock.
            </p>
            <div>
              <label className="label font-semibold text-xs">Recount Instructions & Reason *</label>
              <textarea
                className="input w-full h-24 mt-1 text-xs"
                placeholder="Specify which items or aisles need re-verification..."
                value={recountReason}
