 ...
 * Enterprise Physical Stock Take Management and Audit Console.
 */
...
export default function StockTakeList() {
  ...
  const filtered = useMemo(() => {
    ...
    if (typeFilter !== "ALL") {
      list = list.filter((s) => (s.stock_take_type || "PHYSICAL").toUpperCase() === typeFilter);
    }
    ...
    return list;
  }, [items, searchTerm, typeFilter, statusFilter, warehouseFilter]);
  ...
  async function exportExcel() {
    ...
    const headerRows = buildExcelHeaderRows(headerInfo, {
      title: "PHYSICAL STOCK TAKE SUMMARY",
    ...
    XLSX.utils.book_append_sheet(wb, ws, "StockTakes");
  ...
  async function exportPDF() {
    ...
    let y = applyPdfHeader(doc, headerInfo, {
      ...
      subtitle: `Total Audits: ${filtered.length} | Status Filter: ${statusFilter}`,
      kpis: [
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto p-4 sm:p-6">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              ...
              <p className="text-sm mt-1 opacity-90">
                Enterprise inventory verification, discrepancy reconciliation, and controlled adjustment posting
              </p>
            </div>
            <div className="flex flex-wrap gap-2 items-center">
              ...
              <Link to="/inventory/stock-take/new" className="btn btn-success flex items-center gap-1.5 text-xs font-semibold">
      ...
      <div className="card shadow-sm border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-800 rounded-lg p-5">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-stretch lg:items-center gap-4 mb-4">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <select
              className="input input-sm max-w-[190px]"
              value={typeFilter}
              onChange={(e) => setTypeFilter(e.target.value)}
            >
              <option value="ALL">All Count Types</option>
              <option value="PHYSICAL">Physical Stock Take</option>
          ...
          <table className={"table w-full text-left " + (viewMode === "grid" ? "table-grid-mode" : "")}>
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-700 text-sm">
              {loading ? (
                <tr>
                  <td colSpan="10" className="text-center py-10 text-slate-500">
                    <RefreshCw className="animate-spin inline mr-2" size={16} /> Loading stock takes...
                  </td>
                </tr>
              ) : null}
              ...
              {!loading && !filtered.length ? (
                <tr>
                  <td colSpan="10" className="text-center py-12 text-slate-500">
                    ...
                    No stock takes found matching the selected filters.
                  </td>
                </tr>
              ) : null}
              ...
              {filtered.map((s) => {
                ...
                const isDaily = (s.stock_take_type || "").toUpperCase() === "DAILY";
                ...
                return (
                  <tr key={s.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/60 transition-colors">
                    <td className="py-3 px-3 font-mono font-bold text-brand-700 dark:text-brand-300">
                      <Link to={`/inventory/stock-take/${s.id}`} className="hover:underline">
      ...
      <FloatingCreateButton to="/inventory/stock-take/new" title="New Stock Take" />
    </div>
  );
}
