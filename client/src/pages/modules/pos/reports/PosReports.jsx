          ...
          <div className="card-body">
            <div className="overflow-auto">
              <table className="table">
                ...
                <tbody>
                  {(categoryShowAll ? categoryShare : categoryShare.slice(0, 5)).map((c, idx) => (
                    <tr key={idx}>
            ...
            {categoryShare.length > 5 && (
              <button
                onClick={() => setCategoryShowAll((v) => !v)}
                className="mt-3 text-sm text-brand hover:text-brand-600 font-medium"
              >
                {categoryShowAll ? "Show Less" : `Show More (${categoryShare.length - 5} more)`}
              </button>
            )}
          </div>
        </div>
          ...
          <div className="card-body">
            <div className="overflow-auto">
              <table className="table">
                ...
                <tbody>
                  {(profitGroupShowAll ? profitByGroup : profitByGroup.slice(0, 5)).map((g, idx) => (
                    <tr key={idx}>
            ...
            {profitByGroup.length > 5 && (
              <button
                onClick={() => setProfitGroupShowAll((v) => !v)}
                className="mt-3 text-sm text-brand hover:text-brand-600 font-medium"
              >
                {profitGroupShowAll ? "Show Less" : `Show More (${profitByGroup.length - 5} more)`}
              </button>
            )}
          </div>
        </div>
