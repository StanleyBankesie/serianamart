  ...
  const handlePositionChange = (e) => {
    ...
    if (newPosId) {
      ...
      if (position && position.reports_to_pos_id) {
        const manager = employees.find(emp => emp.pos_id === position.reports_to_pos_id && emp.status === 'ACTIVE');
        if (manager) {
      ...
      <form onSubmit={submit}>
        <div className="card">
          <div className="card-body">
                ...
                <section>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input className="input" value={form.national_id || ""} onChange={(e) => update("national_id", e.target.value)} placeholder="Identity card number" />
                    </div>
                  </div>
                </section>
              ...
              <div className="w-full md:w-64 space-y-4">
                <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-lg border-2 border-dashed border-slate-200 dark:border-slate-700 text-center">
                  <div className="w-32 h-32 mx-auto bg-white dark:bg-slate-900 rounded shadow-inner mb-3 flex items-center justify-center overflow-hidden">
                    {form.picture_url ? (
                      <img src={form.picture_url} alt="Passport" className="w-full h-full object-cover" />
                    ) : (
            ...
            <h3 className="text-lg font-semibold mb-4 border-b pb-2 mt-8">Employment Details</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              ...
              <div>
                ...
                <select className="input" value={form.pos_id || ""} onChange={handlePositionChange}>
              ...
              <div>
                ...
                <select className="input" value={form.employment_type_id || ""} onChange={(e) => update("employment_type_id", e.target.value)}>
              ...
              <div>
                ...
                <select className="input" value={form.manager_id || ""} onChange={(e) => update("manager_id", e.target.value)}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-8">
              <section className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-lg">
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto pr-2">
                  {displayTaxes.map(t => {
                    const isChecked = t.groupIds.some(id => (form.tax_mappings || []).includes(id));
                    return (
                      <label
                      ...
                      >
                        <input
                          ...
                          className="checkbox"
                          checked={isChecked}
                          onChange={() => toggleMapping("tax_mappings", t.groupIds)}
                        />
                        <div className="flex-1">
                          <div className="text-sm font-medium">{t.tax_name}</div>
                  ...
                  {displayTaxes.length === 0 && <p className="text-xs text-slate-400">No tax configurations set.</p>}
                </div>
              </section>
              ...
              <section className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-lg">
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto pr-2">
                  {allowances.map(a => (
                    <label key={a.id} className="flex items-center gap-3 p-2 hover:bg-white dark:hover:bg-slate-700 rounded transition-colors cursor-pointer">
                      <input 
                        type="checkbox" 
                        className="checkbox"
                        checked={form.allowance_mappings.includes(a.id)}
                        onChange={() => toggleMapping("allowance_mappings", a.id)}
                      />
                      <div className="flex-1">
                  ...
                  {allowances.length === 0 && <p className="text-xs text-slate-400">No allowances defined.</p>}
                </div>
              </section>
            </div>
