      ...
      <div className="p-4 space-y-4">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-slate-500">Define salary structures and components</p>
          </div>
          <div className="flex gap-2">
            <Link to="/human-resources" className="btn-secondary">Back to Menu</Link>
            <Link to="/human-resources/salary-config/new" className="btn-primary">+ New Config</Link>
          </div>
        </div>
              ...
              {items.map((r) => (
                <tr key={r.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-right text-sm font-medium">
                    <Link to={`/human-resources/salary-config/${r.id}`} className="text-brand hover:underline">
                      Edit
                    </Link>
                  </td>
                </tr>
              ...
              {items.length === 0 && !loading && (
                <tr>
                  <td colSpan={4} className="px-4 py-12 text-center text-slate-500">
                    No salary structures found. Click "+ New Config" to create one.
                  </td>
                </tr>
              )}
            </tbody>
